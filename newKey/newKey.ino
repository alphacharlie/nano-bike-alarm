/**
 * ----------------------------------------------------------------------------
 * Adapted from the MFRC522 library example; see https://github.com/miguelbalboa/rfid
...
{
    Serial.println();
    Serial.println(F("Provision or Clear MIFARE Classic PICC for the Nano Bike Alarm."));
    Serial.print(F("Using key (for A and B):"));
    dump_byte_array(key.keyByte, MFRC522::MF_KEY_SIZE);
    Serial.println();
    
    Serial.print(F("BEWARE: Data will be written to the PICC, in sector # "));
    Serial.print(F("WARNINIG: DO NOT PROVISION KEYS UNTIL YOU HAVE CuSTOMIZED YOUR KEY AND TOKEN!!!"));
    Serial.println(sector);
    Serial.println(F(""));
    Serial.println(F("Key Provisioning"));
    Serial.println(F("-------------------------------------"));
    Serial.println(F("[p]  Provision Key Card/PICC"));
    Serial.println(F("[u]  Clear/Reset Key Card/PICC (Unprovision)"));
    Serial.println(F("[t]  Test Key Card/PICC Provisioning"));
    Serial.println(F("-------------------------------------"));
}
/**
 * Main loop.
 */
void loop() {
  Serial.read(); //clear any input
  //show prompt
  Serial.print(F("newKey> "));
  ...
  switch (command) {
    ...
    case 'P': {
      ...
      Serial.println(F("OK-  Hold a PICC up to the reader to PROVISION"));
      // Look for new cards
      ...
      else
      {
        ...
        else
        {
          ...
          Serial.print(F("Writing data into token block ")); Serial.print(blockAddr);
          Serial.println(F(" ..."));
      ...
      Serial.read();
      Serial.println(F("Finished! Press any key..."));
      while (!Serial.available()); //wait for key press.
      break;
    }
    ...
    case 'u':
    case 'U': {
      Serial.println(F("OK-  Hold a PICC up to the reader to CLEAR"));
      // Look for new cards
      while ( ! mfrc522.PICC_IsNewCardPresent());
      ...
      Serial.println(F("Attempting Card Unprovision"));  
      // Authenticate using master key
      Serial.println(F("Authenticating using master key..."));
      status = mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, trailerBlock, &newKey, &(mfrc522.uid));
      if (status != MFRC522::STATUS_OK) {
        Serial.print(F("PCD_Authenticate() failed: "));
        Serial.println(mfrc522.GetStatusCodeName(status));
        Serial.println(F("PICC might not be provisioned(or key is wrong)..."));
        while (!Serial.available()); //wait for key press.
      }
      else
      {
        //success with new key, write factory key
        for (byte i = 0; i < 6; i++) {
          //Prepare the key block with our factory key
          byte j = i + 10;
          newKeyBlock[i] = 0xff;
          newKeyBlock[j] = 0xff;
        }
        Serial.read();
        Serial.println(F("PICC detected... Press any key to CLEAR"));
        while (!Serial.available()); //wait for key press.
        // Write data to the trsiler block
        Serial.print(F("Writing data into block ")); Serial.print(trailerBlock);
        Serial.println(F(" ..."));
        ...
        else
        {
          ...
          Serial.println(F(" ..."));
          dump_byte_array(clearBlockData, 16); Serial.println();
          status = mfrc522.MIFARE_Write(blockAddr, clearBlockData, 16);
          if (status != MFRC522::STATUS_OK) {
            Serial.print(F("MIFARE_Write() failed: "));
            Serial.println(mfrc522.GetStatusCodeName(status));
          }
      ...
      if (status != MFRC522::STATUS_OK) {
        Serial.print(F("PCD_Authenticate() failed: "));
        Serial.println(mfrc522.GetStatusCodeName(status));
        Serial.println(F("PICC clear failed. Sector may be pooched!!!"));
        return;
      }    
      ...
      for (byte i = 0; i < 16; i++) {
        // Compare buffer (= what we've read) with dataBlock (= what we've written)
        if (buffer[i] == clearBlockData[i])
            count++;
      }
      Serial.print(F("Number of bytes that match = ")); Serial.println(count);
      if (count == 16) {
        Serial.println();
        Serial.println(F("SUCCESS! Key CLEARED. :-)"));
        Serial.println();
        Serial.println(F("You may now use that PICC for other stuff..."));
        Serial.println();
        Serial.print(F("Don't forget to remove your card UID: '"));
        dump_byte_array(keyUID, 4);
        Serial.println(F("' from the keyUIDs array in the Bike-Alarm sketch!!!"));
      } else {
        Serial.println(F("Failure, no match :-("));
        Serial.println(F("  perhaps the write didn't work properly..."));
      }
    ...
    case 't':
    case 'T': {
      Serial.println(F("OK-  Hold a PICC up to the reader to TEST AUTHENTICATION"));
      while ( ! mfrc522.PICC_IsNewCardPresent());
     ...
     // Authenticate using new Key
      Serial.println(F("Authenticating using saved Key..."));
      byte status = mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, trailerBlock, &newKey, &(mfrc522.uid));
      if (status != MFRC522::STATUS_OK) {
        Serial.print(F("PCD_Authenticate() failed: "));
        Serial.println(mfrc522.GetStatusCodeName(status));
        Serial.println(F("Key may be pooched or unprovisioned!!!"));
        return;
      }    
